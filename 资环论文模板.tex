...
\setCJKfamilyfont{zhsong}[AutoFakeBold = {3}]{SIMSUN.TTC} % 宋体加粗支持
\renewcommand{\zhsong}{\CJKfamily{zhsong}}
\setCJKfamilyfont{kaishu}[AutoFakeBold = {3}]{SIMHEI.TTF} % 黑体加粗支持
\renewcommand{\zhhei}{\CJKfamily{zhhei}}
...
% 毕业设计标题
\begin{center}
    {\zhsong\zihao{-1}  \textbf{本科毕业设计（论文）}}
\end{center}
...
\newcommand{\tableitem}[2]{{\zihao{3}\zhsong \makebox[3.5cm][s]{#1}} & {\zihao{3}\zhsong #2} \\ \cline{2-2}}
\newcommand{\tableitemshort}[2]{{\zihao{3}\zhsong \makebox[3.5cm][s]{#1}} & {\zihao{3}\zhsong #2} \\ \cline{2-2}} % 题目行也应用此格式
...
\chead{\color{gray}\zhsong\zihao{5}兰州资源环境职业技术大学毕业论文} % 设置页眉居中内容，宋体小五灰色
%\cfoot{\thepage} % 页脚中间显示页码 (封面、摘要、目录前不显示页码)

% 论文标题（中文页）
\begin{center}
    {\zihao{-2}\zhsong   XXXXXXXX 的发展现状及应用研究}
\end{center}
...
% 关键词
\noindent % 取消段落缩进
{\zhhei\zihao{4}关键词：}{\zhsong\zihao{-4} XX；XX；XX}
...
\newpage
% 引言
\section*{\centering \zhhei\zihao{-2} \textbf{引\hspace{2em}言}}
\vspace{0.5cm}
\begin{spacing}{1.25} % 设置1.25倍行距
\zhsong\zihao{4} 
...
% 目录
\begin{center}
    {\zhhei\zihao{-2} 目\hspace{2em}录}\vspace* {-2\baselineskip}
\end{center}
\vspace{0.01cm}

% 设置目录格式
\renewcommand{\contentsname}{} % 清除默认的目录标题
\renewcommand{\cfttoctitlefont}{\hfill\zhhei\zihao{3}} % 设置目录标题字体
\renewcommand{\cftaftertoctitle}{\hfill} % 设置目录标题后的格式
\renewcommand{\cftsecfont}{\zhsong\zihao{-4}} % 一级标题设置为宋体小四
\renewcommand{\cftsubsecfont}{\zhsong\zihao{5}} % 二级标题设置为宋体五号
\renewcommand{\cftsecpagefont}{\zhsong\zihao{-4}} % 一级标题页码字体
\renewcommand{\cftsubsecpagefont}{\zhsong\zihao{-4}} % 二级标题页码字体
...
\chead{\color{gray}\zhsong\zihao{5}兰州资源环境职业技术大学毕业论文}
...
\renewcommand{\footrulewidth}{0pt}
\chead{\color{gray}\zhsong\zihao{5}兰州资源环境职业技术大学毕业论文}
\cfoot{\color{gray}\zhsong\zihao{5}第~$\thepage$~页\quad 共~$\pageref{LastPage}$~页} % 设置页脚格式，页码使用数学模式，并添加空格
\thispagestyle{fancy} % 强制当前页为fancy

% \newpage % 由cleardoublepage处理分页
\section{\zhhei\zihao{3}{1\hspace{1em}绪论}}
\renewcommand{\thesection}{}
\begin{spacing}{1.25} % 设置1.25倍行距
\zhsong\zihao{4}
这里是绪论的引言部分，用于介绍研究的背景、目的和意义。\cite{HLKX202511017}

\subsection{\zhhei\zihao{-3}1.1 背景介绍}
\renewcommand{\thesubsection}{}
\zhsong\zihao{4}
详细阐述研究的背景和重要性。

\subsection{\zhhei\zihao{-3}1.2 公式的样例}
\renewcommand{\thesubsection}{}
\zhsong\zihao{4}
综述国内外相关领域的研究进展。

\subsection{\zhhei\zihao{-3}1.3 文章目的与研究意义}
\renewcommand{\thesubsection}{}
\zhsong\zihao{4}
明确本研究的具体内容和采用的研究方法。

\subsection{\zhhei\zihao{-3}1.4 论文结构安排}
\renewcommand{\thesubsection}{}
\zhsong\zihao{4}
简要介绍论文的章节安排。
\end{spacing}
...
% 参考文献
\begin{spacing}{1.25} % 设置1.25倍行距
{\centering \zhhei\zihao{-2} 参考文献\par}\vspace* {-2\baselineskip}
\zhsong\zihao{4}
...
% 致谢
\begin{spacing}{1.25} % 设置1.25倍行距
{\centering \zhhei\zihao{-2} 致谢\par}\vspace* {\baselineskip} % 致谢标题，黑体，小二号，居中，段后增加一些垂直间距
\zhsong\zihao{4} % 正文宋体四号
\noindent % 不缩进
在此处填写致谢内容。
